...
│   │   ├── _app.tsx
│   │   ├── index.tsx   # Referral form UI
│   │   └── api/trpc/[trpc].ts  # tRPC handler
│   ├── server/         # tRPC routers (backend procedures)
│   │   ├── trpc.ts
│   │   └── routers/
│   │       ├── _app.ts
│   │       └── referral.ts  # submitReferral mutation
...
**Rationale**:
- **Pages Router**: Simpler API routes integration for tRPC.
- **tRPC-centric**: Backend procedures mirror frontend forms for zero-config typesafety.
- **src/ monorepo-style**: Clear separation (db/server/frontend).
- **Schema-first**: DB schema drives Zod schemas for validation consistency.
